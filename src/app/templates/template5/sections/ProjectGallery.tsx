      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, i) => {
            ...
            return (
              <BubbleCard key={i} delay={i * 0.1} className="group p-0 overflow-hidden min-h-[400px] flex flex-col bg-zinc-900 border-zinc-800">
                <div className="relative h-48 w-full bg-zinc-800 overflow-hidden border-b border-zinc-800">
                   {project.image ? (
                     <img 
                       src={project.image} 
                       alt={project.title} 
                     ...
                     <div className="absolute inset-0 bg-zinc-800 flex items-center justify-center">
                        <span className="text-4xl font-bold text-zinc-700">{project.title[0]}</span>
                     </div>
                   )}
                </div>
                
                <div className="p-8 flex-grow flex flex-col">
                  <h3 className="text-2xl font-bold mb-3 text-white group-hover:text-yellow-400 transition-colors">{project.title}</h3>
